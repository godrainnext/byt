...
export class AlwaysOpen extends PureComponent {
    ...
    renderContent = () => (
        <View>
            {this.state.data.map((item) => (<View key={item.id} style={s.song}>
                <View style={s.smallbox}>
                    <Text style={{ fontSize: pxToDp(12), color: 'grey' }}>{item.id}</Text>
                    <View style={{ marginLeft: pxToDp(20) }}>
                        <Text style={{ fontSize: pxToDp(14), color: 'black', fontWeight: 'bold' }}>{item.song}</Text>
...
const s = StyleSheet.create({
    ...
    song: {
        ...
        width: pxToDp(350), flexDirection:
            'row', marginTop: pxToDp(10)
    },
    smallbox: {
        flexDirection: 'row',

...
class index extends Component {
        ...
        this.state = {
            ...
            cards: [
                ...
                },
                {
                    ...
                    title: '苎萝浣纱女',
                    context: '诸暨市越剧团的经典原创越剧《西施断缆》在苏州保利大剧院的演出顺利结束'
                },
                {
        ...
        if (this.state.arr != null && this.state.arr.length >= 9) {
            ...
            return (
                <TouchableOpacity
                    ...
                    <Image source={require("../../res/addimg.png")} style={{ width: 100, height: 100 }} />
                </TouchableOpacity>
            );
        }
    }
    ...
    handleSubmit = async () => {
        ...
        axios.post(BASE_URI + '/readerComment/upReaderComment', {
            headimg: headimg,
            user_name: user_name,
            comment: text,
            likeNumber: '0'
    ...
    getList = async () => {
        // axios.get('http://192.168.50.117:3000/readerComment/getReaderComment')
        axios.get(BASE_URI + '/readerComment/getReaderComment')
            .then((json) => {
                this.setState({
                    records: json.data,
                });
    ...
    getIms = async () => {
        // axios.get('http://192.168.50.117:3000/readerComment/getReaderComment')
        axios.get(BASE_URI + '/users/checkUser?user_id=5')
            .then((json) => {
                this.setState({
    ...
    render() {
        ...
        return (
            ...
            >
                <View style={{ flex: 1, backgroundColor: 'transparent', borderRadius: 20, padding: 10, marginTop: pxToDp(80) }}>
                    <View style={{ height: '100%', }}>
                        <View style={{ alignItems: 'flex-end' }}>
                            <TouchableOpacity style={{ width: pxToDp(40), height: pxToDp(40),marginBottom:pxToDp(-17),marginRight:pxToDp(20) }} onPress={toggleModalProps} >
                                <Ionicons
                            ...
                            <View style={{borderColor:'white',borderWidth:pxToDp(1),height:pxToDp(70),marginRight:pxToDp(46)}}>
                            </View>
                        </View>
                                ...
                                renderCard={(card) => {
                                    return (
                                        <View style={styles.card}>
                                            <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={() => this.context.navigate("Comments")}>
                                                <Image style={{ width: '96%', height: pxToDp(160), borderRadius: pxToDp(20), }} source={{ uri: cards[currentIndex].uri }}></Image>
                                                {/* <Text style={styles.text}>{card}</Text> */}
                                                {/* 标题名 */}
                                                <View style={{ width: 200, height: 58, justifyContent: 'center', alignItems: 'center', }}>
                                                ...
                                                {/* 分割线 */}
                                                <View>
                                                    <Text style={{ textAlign: 'center', color: '#F0bb51', fontWeight: 'bold' }}>
                                                        _________________
                                                    </Text>
                                                </View>
                                            </TouchableOpacity>

        ...
        {this.props.userinfo.collect.map((item) => (
          <View key={item.collectid} style={{ elevation: 1, borderWidth: 0, flex: 1, width: '95%', alignSelf: 'center', backgroundColor: 'white', borderRadius: pxToDp(10), marginTop: pxToDp(10) }}>
            ...
            <View style={{ flexDirection: 'row', marginTop: pxToDp(10), }}>
              ...
              <Text style={{ width: '50%', paddingLeft: pxToDp(20), fontSize: pxToDp(15) }} numberOfLines={2}>{item.title}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', position: 'absolute', bottom: pxToDp(10), left: pxToDp(132) }}>
                <SvgUri svgXmlData={playnumber} width='15' height='15' />
                <Text style={{ marginLeft: pxToDp(10), fontSize: pxToDp(12) }}>{item.playnum}</Text>
              </View>
              <SvgUri svgXmlData={pinglun} width='16' height='16' style={{ bottom: pxToDp(8), left: pxToDp(65) }} />
              <TouchableOpacity style={{ position: 'absolute', right: pxToDp(10), bottom: pxToDp(10) }}>
                <SvgUri svgXmlData={point} width='15' height='15' />
              </TouchableOpacity>
              <View style={{ position: 'absolute', bottom: pxToDp(55), right: pxToDp(10) }}>
                <Chip
                  title={item.label}
                  size={10}
                  color='grey'
